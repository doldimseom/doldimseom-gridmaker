...
function makeBlk(blk) {
  ...
  if (blk.type === 'img') {
    ...
    textArea.addEventListener('input', (function(b, texEl, parentEl) { return function() {
      ...
      applyVAlignAll();
      /* 너비 자동 늘어남 — txt는 줄바꿈으로, item은 _itemAutoExpand가 별도 처리하므로 제외 */
      if (b.type !== 'txt' && b.type !== 'item') {
  ...
  el.onclick = (function(k, bt, b, curEl) { return function(e) {
    ...
    if (b.groupId) {
      if (b.groupId === selectedGi) {
        ...
        /* 같은 블록 재클릭(더블클릭 2번째 onclick 포함) 시 개별 모드 유지
           다른 블록 클릭 시에만 초기화 */
        if (selKey !== k) _grpIndividualMode = false;
        selectedGi = null;
        hideGroupToolbar();
      }
    }
